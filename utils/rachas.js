...
function calcularMultiplicador(racha) {
    if (racha >= 14) return 3;
    if (racha >= 7) return 2;
    if (racha >= 3) return 1.5;
    return 1;
}

async function actualizarRachaJuego(usuarioId) {
    const hoy = new Date().toISOString().slice(0, 10);
    const existente = await db.query(
        'SELECT id, racha_actual, racha_maxima, ultimo_activo FROM rachas WHERE usuario_id = $1',
        [usuarioId]
    );

    if (existente.rows.length === 0) {
        await db.query(
            `INSERT INTO rachas (usuario_id, racha_actual, racha_maxima, ultimo_activo)
             VALUES ($1, 1, 1, $2)`,
            [usuarioId, hoy]
        );
        return 1;
    ...
    const ultimo = r.ultimo_activo ? r.ultimo_activo.toISOString().slice(0, 10) : null;

    if (ultimo === hoy) return r.racha_actual;
    ...
    if (ultimo) {
        ...
        if (ultimo === ayer.toISOString().slice(0, 10)) {
            nuevaRacha = r.racha_actual + 1;
        }
    }

    const nuevaMaxima = Math.max(nuevaRacha, r.racha_maxima);
    await db.query(
        `UPDATE rachas SET racha_actual = $1, racha_maxima = $2, ultimo_activo = $3, updated_at = NOW()
         WHERE id = $4`,
...
module.exports = { calcularMultiplicador, actualizarRachaJuego, actualizarRachaCreacion };